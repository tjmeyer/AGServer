...
bool network::databaseInit()
{
    ...
    if(!db.isOpen())
    {
        ...
        else
        {
            ...
            out << "\tOptions set: " << db.connectOptions() << endl;
        }
    }
    return db.isValid();
}

QSqlQuery network::dbExec(QString sql)
{
    QTextStream out (stdout);
    QSqlQuery query;
    if(db.isOpen())
    ...
    {
        out << "Detected Database Connection Error:\n\t" << db.lastError().text() << endl;
        out << "\tReinitializing database...\n";
        if(databaseInit())
        {
            query = db.exec(sql);
        }
        ...
        {
            out << "\t\t!!!Database Initialization failure!!! exiting.\n";
            exit(1);
        }
    }
    return query;
...
void network::authReader()
{
    ...
    if(socket->isValid())
    {
        ...
        case packetType::SYSTEM_REQUEST:
            ...
            {
                QJsonObject system_packet = getSystem(packet.value(QString("id")).toString());
        ...
        case packetType::GET_USER_SYSTEM_ID:
            ...
            {
                QString sql = "SELECT id, sector_id FROM system WHERE user_id = " + packet.value(QString("user_id")).toString();
                QSqlQuery query = dbExec(sql);
                QJsonObject packet;
                if(query.next())
                {
...
bool network::authenticate(QTcpSocket *socket, QString username, QString password)
{
    ...
    {
        QTextStream out(stdout);
        QSqlQuery query = dbExec("SELECT password, id FROM users WHERE username = '" + username +"'");
        if(query.next())
        {
            ...
            if(query.value(0).toString() == password)
            {
                ...
                packet.insert(QString("user_id"), QJsonValue(query.value(1).toString()));
...
QJsonArray network::getPlanets(QString star_id)
{
...
QJsonObject network::getStar(QString star_id)
{
    QJsonObject star;
...
QJsonObject network::getSystem(QString id)
{
